...
class LogEntryProcessor:
    ...
    """
    # This is a good host indicator whether deployed locally or in a k8s pod
    ...
    def get_request_id() -> str:
        """
        Get the 'x-request-id' associated with this request
        """
    ...
    def set_request_id(request_id: str) -> None:
        """
        Logging middleware sets the 'x-request-id' and we store in TLS so we
        can include it in every log entry.
        """
    ...
    def add_timestamp(_, __, event_dict: dict) -> dict:
        ...
        TODO: old requirement to specify time in millis - update this
        Analytics requires timestamps to be of form: YYYY-MM-DDTHH:MM:SS.sssZ
        ...
        ELK requires the @timestamp field name, or it will insert its own
        """
        now = datetime.datetime.utcnow()
        millis = '{:03d}'.format(int(now.microsecond / 1000))
        event_dict["@timestamp"] = "{}.{}Z".format(now.strftime('%Y-%m-%dT%H:%M:%S'), millis)
        return event_dict
